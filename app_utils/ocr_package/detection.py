...
def batch_text_detection(images: List[Image.Image], rapidocr_detector: TextDect_withRapidocr) -> List[TextDetectionResult]:
    ...
    for img in images:
        ...
        for bbox_coords in det_results:
            try:
                # Adjust the bounding box height by 0.5 (expand height by 50%)
                adjusted_bbox_coords = adjust_bbox_height(bbox_coords, height_adjustment_factor=0.1)
                # Create a PolygonBox object from the adjusted bbox coordinates
