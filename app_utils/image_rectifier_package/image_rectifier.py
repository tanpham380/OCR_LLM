...
class ImageRectify:
    ...
    @staticmethod
    @torch.jit.script
    def expand_box(box: torch.Tensor, image_shape: Tuple[int, int], crop_expansion_factor: float) -> torch.Tensor:
        h, w = image_shape[:2]
        x1, y1, x2, y2 = box
        ...
        x1_new = max(0.0, x1 - box_width * crop_expansion_factor)
        y1_new = max(0.0, y1 - box_height * crop_expansion_factor)
        x2_new = min(float(w), x2 + box_width * crop_expansion_factor)
        y2_new = min(float(h), y2 + box_height * crop_expansion_factor)
